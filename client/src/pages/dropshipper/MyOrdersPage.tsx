...
const MyOrdersPage = () => {
  ...
  return (
    <div className="relative animate-[fadeIn_0.2s_ease] text-white">
      <h1 className="text-3xl font-bold text-white">My Orders</h1>
      <p className="mt-1 text-zinc-400">Track all your placed orders.</p>
      ...
      {isLoading ? (
        <div className="mt-4">
          {Array.from({ length: 5 }).map((_, index) => (
            <div key={index} className="mb-3 h-16 animate-pulse rounded-xl bg-[#1a1a1a]" />
          ))}
        </div>
      ) : orders.length === 0 ? (
        <div className="mt-20 flex flex-col items-center text-center">
          <Package className="h-16 w-16 text-zinc-600" />
          <p className="mt-4 text-lg text-zinc-500">No orders found</p>
          <p className="mt-1 text-sm text-zinc-600">Start by ordering from the catalog</p>
          <button
            type="button"
            onClick={() => navigate('/catalog')}
            className="mt-5 rounded-lg bg-orange-500 px-4 py-2 text-sm font-medium text-white"
          >
            Browse Catalog
          </button>
        </div>
              ...
              <tbody>
                {orders.map((order) => {
                  ...
                  return (
                    <tr
                      key={order._id}
                      className="border-b border-zinc-800 transition hover:bg-[#1f1f1f] last:border-0"
                    >
                      <td className="px-4 py-3 font-mono text-sm font-medium text-orange-500">
                        {order.orderId}
                      </td>
                        ...
                        <div className="flex items-center gap-3">
                          {imageUrl ? (
                            <img
                              ...
                              className="h-10 w-10 rounded-lg bg-[#111111] object-cover"
                              onError={handleImageError}
                            />
                          ) : (
                            <div className="h-10 w-10 rounded-lg bg-[#111111]" />
                          )}
                          <p className="max-w-32 truncate text-sm text-white">{order.productId?.name}</p>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 text-sm font-medium text-white">{order.quantity}</td>
                      <td className="px-4 py-3 text-sm text-zinc-400">
                      ...
                      <td className="px-4 py-3">
                        <button
                          ...
                          className="cursor-pointer text-xs text-orange-500 hover:underline"
                        >
                          View Details
                        </button>
                      </td>
      ...
      <aside
        className={`fixed right-0 top-0 z-50 h-full w-96 border-l border-zinc-800 bg-[#111111] p-5 transition-transform ${
      ...
      >
        ...
        {selectedOrder ? (
          <div className="mt-6 space-y-4 text-sm">
            <p className="font-mono text-orange-500">{selectedOrder.orderId}</p>
            ...
            <div className="flex items-center gap-3">
              {selectedOrder.productId?.media?.[0]?.url ? (
                ...
                <div className="h-14 w-14 rounded-lg bg-[#1a1a1a]" />
              )}
              <p className="text-white">{selectedOrder.productId?.name}</p>
            </div>

            <div className="rounded-xl bg-[#1a1a1a] p-4 text-zinc-300">
              <p>
                <span className="text-zinc-500">Name:</span> {selectedOrder.customerName}
              </p>
              <p className="mt-2">
                <span className="text-zinc-500">Phone:</span> {selectedOrder.customerPhone}
              </p>
              <p className="mt-2">
                <span className="text-zinc-500">Address:</span> {selectedOrder.customerAddress}
              </p>
              <p className="mt-2">
                <span className="text-zinc-500">Pincode:</span> {selectedOrder.customerPincode}
              </p>
            </div>

            <div className="text-zinc-300">
              <p>
                <span className="text-zinc-500">Quantity:</span> {selectedOrder.quantity}
              </p>
              {selectedOrder.note ? (
                <p className="mt-2">
                  <span className="text-zinc-500">Note:</span> {selectedOrder.note}
                </p>
              ) : null}
            </div>

            <div className="mt-6">
              <p className="mb-4 text-sm font-medium text-zinc-400">Order Timeline</p>
                  ...
                  {timelineSteps.map((step, index) => {
                    ...
                    return (
                      <div key={step} className="relative flex gap-3 pb-5 last:pb-0">
                        {index < timelineSteps.length - 1 ? (
                          <span
                            className={`absolute left-[13px] top-7 h-[calc(100%-12px)] w-[2px] ${
                              currentStatusIndex > index ? 'bg-orange-500' : 'bg-zinc-800'
                            }`}
                          />
                        ) : null}
                        <span
                        ...
                        <p className={`${isCompleted ? 'text-white' : 'text-zinc-600'} capitalize`}>{step}</p>
                      </div>
                    )
                  })}
                </div>
              ...
              {selectedOrder.trackingNumber ? (
                <div className="mt-4 rounded-lg bg-[#1a1a1a] p-3">
                  ...
                  <p className="font-mono text-white">{selectedOrder.trackingNumber}</p>
                </div>
              ) : null}
              ...
              {selectedOrder.adminNote ? (
                <div className="mt-3">
                  ...
                  <p className="text-sm text-zinc-300">{selectedOrder.adminNote}</p>
                </div>
              ) : null}
            </div>
          </div>
