...
const NotificationsPage = () => {
  ...
  return (
    <div className="animate-[fadeIn_0.2s_ease] text-white">
      ...
      <p className="mt-1 text-zinc-400">Stay updated on your orders.</p>
      ...
      {isLoading ? (
        <div className="mt-4 space-y-2">
          {Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="h-20 animate-pulse rounded-xl bg-[#1a1a1a]" />
          ))}
        </div>
            ...
            >
              <div
                className={`mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${
                  notification.isRead ? 'bg-zinc-800 text-zinc-600' : 'bg-orange-500/20 text-orange-500'
                }`}
              >
              ...
              {!notification.isRead ? <span className="mt-1 h-2 w-2 rounded-full bg-orange-500" /> : null}
            </article>
          ))}
        </div>
      )}
