...
const AdminDashboard = () => {
  ...
  const handleSaveDesignKitPdf = async () => {
    ...
    try {
      const res = await uploadDesignKitPdf(designKitPdfFile)
      toast.success('Design kit PDF uploaded')
      setDesignKitPdfUrl(res.data.designKitUrl)
    ...
    <div className="animate-[fadeIn_0.2s_ease] text-foreground">
      ...
      <section className="mt-6 rounded-xl border border-border bg-surface-secondary p-6">
        ...
        <p className="mt-2 text-sm text-foreground-muted">
          Upload a new PDF to replace the current design kit.
        </p>
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <label className="flex-1 cursor-pointer rounded-lg border border-dashed border-border bg-surface px-4 py-2.5 text-center text-sm text-foreground-muted transition hover:border-primary hover:text-primary">
            <Upload className="mx-auto mb-1 h-5 w-5" />
            <span>{designKitPdfFile ? designKitPdfFile.name : 'Select PDF File'}</span>
            <input
              type="file"
              accept="application/pdf"
              onChange={(e) => setDesignKitPdfFile(e.target.files?.[0] ?? null)}
              className="hidden"
            />
          </label>
          <button
            ...
            className="rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-foreground disabled:opacity-60"
          >
            {isSavingDesignKitPdf ? 'Uploading...' : 'Upload & Save'}
          </button>
        </div>
        {designKitPdfUrl && (
          <a
            href={getImageUrl(designKitPdfUrl)}
            target="_blank"
            rel="noreferrer"
