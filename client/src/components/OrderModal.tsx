      ...
      <div className="absolute left-1/2 top-1/2 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 p-4">
        <div className="rounded-2xl border border-border bg-surface-secondary p-6">
          <div className="relative">
            ...
            <p className="mt-1 pr-10 text-sm text-foreground-muted">{product.name}</p>
            <button
              type="button"
              onClick={onClose}
              className="absolute right-0 top-0 text-foreground-muted transition hover:text-foreground"
            >
              <X className="h-5 w-5" />
            </button>
          </div>

          <div className="mt-4 flex gap-3 rounded-xl bg-surface p-3">
            {productImage ? (
              <img src={getImageUrl(productImage)} alt={product.name} className="h-16 w-16 rounded-lg object-cover" onError={handleImageError} />
            ) : (
              <div className="h-16 w-16 rounded-lg bg-surface-secondary" />
            )}
            <div>
              <p className="text-sm font-medium text-foreground">{product.name}</p>
              <p className="mt-1 text-sm text-primary">₹{product.dropshipperPrice}/unit</p>
              <p className="mt-1 text-xs text-foreground-muted">Min order: {product.moq} units</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-3">
            <input
              ...
              className="w-full rounded-lg border border-border bg-surface px-4 py-3 text-sm text-foreground placeholder-zinc-500 focus:border-primary focus:outline-none"
            />
            <input
              type="tel"
            ...
            <button
              ...
              className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-[#25D366] py-4 font-bold text-foreground transition hover:bg-green-600 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {isSubmitting ? (
