...
const buildPdfMedia = (baseUploadsUrl, files, sortOrder) => {
    if (!files || files.length === 0) {
        return [];
    }
    return files.map((file, index) => ({
...
const getAllProducts = async (req, res) => {
    try {
        ...
        if (typeof kind === 'string' && kind.trim()) {
            const categories = await Category_model_1.default.find({ kind: kind.trim() }).select('_id');
        ...
        const products = await Product_model_1.default.find(query).populate('category', 'name slug kind').sort(sort);
...
const createProduct = async (req, res) => {
    try {
        const { name, description, category, mrp, dropshipperPrice, moq, weight, dimensions, material, stockStatus, downloadableUrl, contentType, sizes } = req.body;
...
const updateProduct = async (req, res) => {
    try {
        ...
        if (downloadableUrl !== undefined)
            product.downloadableUrl = downloadableUrl;
