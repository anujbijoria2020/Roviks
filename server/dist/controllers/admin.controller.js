...
exports.uploadDesignKit = exports.getDropshipperOrders = exports.blockDropshipper = exports.approveDropshipper = exports.getAllDropshippers = exports.getStats = void 0;
...
const uploadDesignKit = async (req, res) => {
    try {
        if (!req.file) {
            res.status(400).json({ message: 'No file uploaded' });
            return;
        }
        const designKitUrl = `/uploads/${req.file.filename}`;
        await Settings_model_1.default.findOneAndUpdate({}, { designKitUrl }, { upsert: true, new: true });
        res.status(200).json({ message: 'Design kit uploaded successfully', designKitUrl });
    }
    catch (error) {
        console.error('Upload Design Kit Error:', error);
        res.status(500).json({ message: 'Server error' });
    }
};
exports.uploadDesignKit = uploadDesignKit;
