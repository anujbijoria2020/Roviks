...
const login = async (req, res) => {
    try {
        ...
        delete userObj.shopify?.accessToken;
...
const getMe = async (req, res) => {
    try {
        ...
        const user = await User_model_1.default.findById(req.user.id).select('-password -shopify.accessToken');
        if (!user) {
            res.status(404).json({ message: 'User not found' });
            return;
        }
...
const updateMe = async (req, res) => {
    try {
        ...
        const user = await User_model_1.default.findByIdAndUpdate(req.user.id, { fullName, phone, whatsappNumber, city, socialHandle }, { new: true, runValidators: true }).select('-password -shopify.accessToken');
        if (!user) {
            res.status(404).json({ message: 'User not found' });
            return;
        }
